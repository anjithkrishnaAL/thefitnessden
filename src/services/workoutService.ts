...
export async function assignWorkoutPlan(memberId: string, planId: string, startDate: string, endDate: string | null) { const { error } = await supabase.from('workout_plan_members').upsert({ member_id: memberId, workout_plan_id: planId, start_date: startDate, end_date: endDate, status: 'active' }, { onConflict: 'workout_plan_id,member_id' }); if (error) throw new Error(error.message); const [{ data: member }, { data: plan }] = await Promise.all([supabase.from('members').select('full_name').eq('id', memberId).maybeSingle(), supabase.from('workout_plans').select('name').eq('id', planId).maybeSingle()]); void createNotification({ title: 'Workout Plan Assigned', message: `${plan?.name ?? 'A workout plan'} was assigned to ${member?.full_name ?? 'a member'}.`, type: 'Workout Plan', member_id: memberId, dedupe_key: `workout:${memberId}:${planId}` }).catch(err => console.error('[workoutService.notification]', err)); }

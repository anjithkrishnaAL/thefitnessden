...
const INITIAL_FILTERS: MemberFilters = { search: '', status: '', gender: '', fitness_goal: '', trainer_id: '', workout_plan_id: '' };
...
export function MembersPage() {
  ...
  const [workoutPlanOptions, setWorkoutPlanOptions] = useState<{ value: string; label: string }[]>([]);
  ...
  useEffect(() => { getWorkoutPlans({ search: '', status: '', goal: '', difficulty: '' }).then(rows => setWorkoutPlanOptions(rows.map(p => ({ value: p.id, label: p.name })))).catch(() => setWorkoutPlanOptions([])); }, []);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="bg-den-card border border-den-border rounded-xl p-3 sm:p-4"><MemberFiltersBar filters={filters} onChange={changeFilters} onClear={() => changeFilters(INITIAL_FILTERS)} trainerOptions={trainerOptions} workoutPlanOptions={workoutPlanOptions} /></div>

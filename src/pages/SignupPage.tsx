...
const FEATURES = [
  { icon: <Dumbbell size={16} />, label: 'Track every member\'s progress' },
  { icon: <Activity size={16} />, label: 'Real-time attendance insights' },
  { icon: <TrendingUp size={16} />, label: 'Revenue & growth analytics' },
];
...
export function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setFormError(err instanceof Error ? err.message : 'Could not create account. Please try again.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="min-h-screen flex overflow-hidden bg-den-bg">
          ...
          <div className="flex-1 flex flex-col justify-center max-w-md">
            <div className="mb-8">
              ...
              <p className="text-base text-den-muted mt-4 leading-relaxed">
                Set up your TheFitnessDen account and start managing your gym like a professional in minutes.
              </p>
            </div>
      ...
      <div className="flex flex-col items-center justify-center flex-1 lg:max-w-md xl:max-w-lg p-6 sm:p-10 overflow-y-auto">
        {/* Mobile brand */}
        ...
        <div className="w-full max-w-sm">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-den-text">Create your account</h1>
            <p className="text-sm text-den-muted mt-1.5">Get started with TheFitnessDen today</p>
          </div>
          ...
          <p className="text-center text-sm text-den-muted">
            ...
            <Link to="/login" className="text-den-accent hover:text-den-accent-dim font-medium transition-colors">
              Sign in
            </Link>
          </p>
        </div>
